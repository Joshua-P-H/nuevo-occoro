<template>
  <Dashboard>
    ...
    >
      Agregar imagenes para el carrousel
    </button>
    ...
    <table class="custom-table">
      ...
      <tbody>
        <tr v-for="documento in carrouseles" :key="documento.id">
          <td>{{ documento.id }}</td>
          <td>{{ documento.name_carrousel }}</td>
          <td>
            <img :src="documento.image_carrousel" alt="Imagen" style="max-width: 100px; max-height: 100px;">
          </td>
          <td>
...
import Modal from "@/Pages/Administrador/Carrousel/Componentes/AgregarModal.vue";
import EditModal from "@/Pages/Administrador/Carrousel/Componentes/EditModal.vue";
...
const { carrouseles } = defineProps(["carrouseles"]);
...
const eliminarDocumento = () => {
  if (
    ...
    try {
      ...
      router.delete(`/carrousel/${documentoId}`);
      closeDeleteModal();
    } catch (error) {
