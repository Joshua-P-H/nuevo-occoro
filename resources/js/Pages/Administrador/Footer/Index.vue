<template>
      ...
      <table class="custom-table">
        ...
        <tbody>
          <tr v-for="footer in footers" :key="footer.id">
            <td>{{ footer.id }}</td>
            <td>{{ footer.gestion_footer }}</td>
            <td>
              <img :src="footer.image_footer" alt="Imagen" style="max-width: 100px; max-height: 100px;">
  
            </td>
            <td>
              <select @change="handleOption(footer, $event.target.value)">
  ...
  import AgregarModal from "@/Pages/Administrador/Footer/Componentes/AgregarModal.vue";
  import EditModal from "@/Pages/Administrador/Footer/Componentes/EditModal.vue";
  ...
  function openEditModal(footer) {
    seleccionarDocumento.value = footer;
  ...
  const openDeleteModal = (footer) => {
    ...
    nombreUsuarioSeleccionado = footer.gestion_footer;
  };
  ...
  const eliminarDocumento = () => {
    if (
      ...
      try {
        ...
        router.delete(`/footers/${documentoId}`);
        closeDeleteModal();
      } catch (error) {
  ...
  function handleOption(footer, option) {
    if (option === "edit") {
      openEditModal(footer);
    } else if (option === "delete") {
      openDeleteModal(footer);
    }
    resetDropdown();
  }
