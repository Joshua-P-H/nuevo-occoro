<template>
      ...
      <div class="modal-fondo" @click="closeModal()">
        <div class="modal-content" @click.stop>
          ...
          <form @submit.prevent="submit" enctype="multipart/form-data">
            <div class="my-3">
              <input v-model="form.name_documento" type="text" id="name_documento"
                     class="rounded-lg w-full p-2.5 dark:focus:ring-blue-500">
            </div>
            <div class="my-3">
              <input type="file" @change="handleImageUpload" id="image_documento" accept="image/*">
            </div>
            <div class="my-3">
              <input type="file" @change="handlePdfUpload" id="pdf_documento" accept=".pdf">
            </div>
            <div class="form-group my-3">
              <button :disabled="form.processing" type="submit" class="bg-green-700 text-white px-4 py-2 rounded">
                Guardar
              </button>
            </div>
          </form>
  
          <div v-if="form.errors.name_documento" class="error-dialogue">
  ...
  const validateForm = () => {
    ...
    if (!form.name_documento) {
      form.errors.name_documento = "El nombre del documento es obligatorio";
      isValid = false;
      setTimeout(() => {
