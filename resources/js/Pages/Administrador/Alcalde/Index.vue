<template>
  <Dashboard>
    ...
    >
      Agregar información del alcalde
    </button>
    ...
    <table class="custom-table">
      ...
      <tbody>
        <tr v-for="alcalde in alcaldes" :key="alcalde.id">
          <td>{{ alcalde.id }}</td>
          <td>{{ alcalde.name_alcalde }}</td>
          <td>
            <img :src="alcalde.image_alcalde" alt="Imagen" style="max-width: 100px; max-height: 100px;">
          </td>
          <td>{{ alcalde.content_alcalde }}</td>

          <td>
            <select @change="handleOption(alcalde, $event.target.value)">
...
import Modal from "@/Pages/Administrador/Alcalde/Componentes/AgregarModal.vue";
import EditModal from "@/Pages/Administrador/Alcalde/Componentes/EditModal.vue";
...
const { alcaldes } = defineProps(["alcaldes"]);
...
function openEditModal(alcalde) {
  seleccionarDocumento.value = alcalde;
...
const openDeleteModal = (alcalde) => {
  seleccionarDocumento.value = alcalde;
  showDeleteModal.value = true;
  nombreUsuarioSeleccionado = alcalde.name_alcalde;
};
...
const eliminarDocumento = () => {
  if (
    ...
    try {
      ...
      router.delete(`/alcalde/${documentoId}`);
      closeDeleteModal();
    } catch (error) {
...
function handleOption(alcalde, option) {
  if (option === "edit") {
    openEditModal(alcalde);
  } else if (option === "delete") {
    openDeleteModal(alcalde);
  }
  resetDropdown();
}
